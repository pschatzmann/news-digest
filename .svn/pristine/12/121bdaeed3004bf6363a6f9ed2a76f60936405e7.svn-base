...
import ch.pschatzmann.news.HistoryDataGuardian;
 ...
 *
 */
public class TestGuardian implements IDocumentStore {
	private int count;
	
	@Test
	public void test() {
		HistoryDataGuardian data = new HistoryDataGuardian(this);
		data.save("2018");
	...
	public void add(Document headline) {
		...
		Assert.assertEquals("guardian", headline.getPublisher_t());
		count++;
	}
